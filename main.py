...
# Session khusus Telegram — terpisah agar tidak rebutan koneksi
tg_session = requests.Session()
tg_adapter = requests.adapters.HTTPAdapter(
    pool_connections=3,
    pool_maxsize=5,
    max_retries=3,
)
...
def _tg_send(text):
    try:
        ...
        data = r.json()
        if not data.get("ok"):
            add_log("ERROR", f"Telegram gagal: {data.get('description', 'unknown')}")
    except requests.exceptions.ConnectionError:
        add_log("ERROR", "Telegram: Tidak bisa konek (cek internet/BOT_TOKEN)")
    except requests.exceptions.Timeout:
        add_log("ERROR", "Telegram: Request timeout")
