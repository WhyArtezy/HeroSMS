...
def build() -> Table:
    """
    Renders everything into a single vertical Table (no Layout),
    works well on narrow Termux screens.
    """
    ...
    bar      = (
        "[bright_green]" + "|" * blocks + "[/bright_green]"
        + "[dim bright_black]" + "." * (20 - blocks) + "[/dim bright_black]"
        + f"  {done_pct:.0f}%"
    )
    ...
    stat.add_column("k", style="dim",          ratio=1)
    stat.add_column("v", style="bright_white", ratio=1)
    stat.add_column("k", style="dim",          ratio=1)
    stat.add_column("v", style="bright_white", ratio=1)

    stat.add_row(
        "Balance",  Text(str(balance),      style="gold1"),
        "Target",   Text(str(JUMLAH_NOMOR), style="bright_white"),
    )
    stat.add_row(
        "Success",  Text(str(success), style="bold bright_green"),
        "Retry",    Text(str(retry),   style="yellow"),
    )
    stat.add_row(
        "Cancel",   Text(str(cancel),  style="red"),
        "Progress", Text(bar),
    )

    root.add_row(
        Panel(stat, border_style="bright_black", padding=(0, 0))
    )
    ...
    num_tbl.add_column("#",       justify="center", width=3,  style="dim")
    num_tbl.add_column("NOMOR",   justify="left",   min_width=13)
    num_tbl.add_column("HARGA",   justify="right",  width=6,  style="yellow")
    num_tbl.add_column("OTP",     justify="center", width=10, style="bold bright_green")
    ...
    if rows_snap:
        for i, r in enumerate(rows_snap, 1):
            num_tbl.add_row(
                ...
                f"{r['price']:.2f}",
                r["otp"],
                Text(r["status"], style=STATUS_COLOR.get(r["status"], "dim")),
            )
    else:
        num_tbl.add_row("-", "[dim]menunggu nomor...[/dim]", "-", "-", "-")

    root.add_row(
        Panel(num_tbl, title="[dim bright_green]NOMOR[/dim bright_green]",
    ...
    for t, lvl, msg in reversed(recent):
        log_tbl.add_row(
            ...
            Text(msg, style="dim white"),
        )

    root.add_row(
        Panel(log_tbl, title="[dim bright_green]LOG[/dim bright_green]",
              border_style="bright_black", padding=(0, 0))
    )
